...
export function List() {
  ...
  const columns: ColumnsType<Application> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size={0}>
          <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/applications/${record.app_id}`)}>查看</Button>
          <Button type="link" size="small" icon={<EditOutlined />} onClick={() => navigate(`/applications/${record.app_id}/edit`)}>编辑</Button>
        </Space>
      ),
    },
  ]
    ...
    <div className={styles.container}>
      <Card>
        <div className={styles.header}>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/applications/create')}>新建应用</Button>
        </div>
